...
// Retrieve User Id session variables
...
// Process wishlist addition if form is submitted
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['listingId']) && isset($_POST['UserId'])) {
    $listingId = intval($_POST['listingId']);
    $UserId = intval($_POST['UserId']);

    // Check if the listing already exists in the wishlist
    $checkSql = "SELECT * FROM wishlist WHERE UserId = ? AND listingId = ?";
    ...
    if ($checkResult->num_rows > 0) {
        echo "Listing is already in your wishlist.";
    } else {
        // Add to wishlist
        $sql = "INSERT INTO wishlist (UserId, listingId) VALUES (?, ?)";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("ii", $UserId, $listingId);

        if ($stmt->execute()) {
            echo "Listing added to wishlist.";
        } else {
            ...
            <div class="row">
                <div class="col-2">
                    <select class="form-select filter" id="propType" name="propType">
                        ...
                        <option value="mountainside_cottage">Mountainside Cottage</option>
                        <option value="underwater_cove">Underwater Cove</option>
